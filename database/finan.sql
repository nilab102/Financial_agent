...
CREATE TABLE AuditLogs (
    ...
    ActionType TEXT NOT NULL CHECK(ActionType IN (
        ...
        'Login', 'Logout', 'FailedLogin', 'System' -- Add other allowed types
    )),
    OldValue TEXT,
    NewValue TEXT,
    ChangedBy INTEGER,
...
CREATE TABLE Products (
    ...
    UnitPrice REAL NOT NULL,                    -- Standard selling price per unit
    IsActive INTEGER NOT NULL DEFAULT 1         -- Boolean flag (1=active, 0=inactive)
);

-- =============================================
-- Warehouse Table (Needed for StockMovements)
-- =============================================
...
CREATE TABLE InventoryItems (
    ...
    StandardCost REAL NOT NULL,                     -- Standard or average cost per unit
    ...
    ReorderPoint REAL,                              -- Minimum quantity level that triggers reordering
...
CREATE TABLE StockMovements (
    ...
    MovementType TEXT NOT NULL CHECK(MovementType IN ('Purchase', 'Sale', 'Adjustment-In', 'Adjustment-Out', 'Transfer-In', 'Transfer-Out')), -- Type of movement
    QuantityChange REAL NOT NULL,                 -- Change in quantity (+ for in, - for out)
    UnitCost REAL,                                -- Cost per unit for this specific movement (important for costing)
...
CREATE TABLE FixedAssets (
    ...
    PurchaseCost REAL NOT NULL,                     -- Original cost of the asset
    SalvageValue REAL NOT NULL DEFAULT 0,           -- Estimated residual value at the end of useful life
    DepreciationMethod TEXT NOT NULL CHECK(DepreciationMethod IN ('Straight-line', 'Declining Balance', 'Sum-of-Years-Digits')), -- Method used for calculating depreciation
    ...
    AccumulatedDepreciation REAL NOT NULL DEFAULT 0,-- Total depreciation recorded to date
    CurrentValue REAL GENERATED ALWAYS AS (PurchaseCost - AccumulatedDepreciation) STORED, -- Current book value (calculated)
    ...
    Status TEXT NOT NULL DEFAULT 'Active' CHECK(Status IN ('Active', 'Disposed', 'Sold')), -- Status of the asset
    DisposalDate TEXT,                              -- Date asset was disposed or sold (ISO format YYYY-MM-DD)
    DisposalPrice REAL,                             -- Amount received upon disposal/sale
